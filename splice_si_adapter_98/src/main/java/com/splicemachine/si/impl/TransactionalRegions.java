...
import com.splicemachine.si.impl.checkpoint.AsyncCheckpointResolver;
...
import com.splicemachine.si.impl.checkpoint.SharedCheckpointResolver;
...
public class TransactionalRegions{
    ...
    private TransactionalRegions(){
    } //can't make a utility class

    private static final Object lock=new Integer("3");
    private static volatile AsyncReadResolver readResolver;
    private static volatile AsyncCheckpointResolver checkpointResolver;
    private static volatile SharedCheckpointResolver sharedCheckpointResolver;
    ...
    public static RollForwardManagement getRollForwardManagement(){
        return status;
    }

    public static void setActionFactory(ActionFactory factory){
        actionFactory=factory;
    }

    public static void setTrafficControl(TrafficControl control){
    ...
    public static TransactionalRegion nonTransactionalRegion(HRegion region){
        return new TxnRegion(region,
                ...
                NoOpCheckpointResolver.INSTANCE);
    }

    public static RollForwardStatus getRollForwardStatus(){ return status; }
    ...
    private static TransactionalRegion get(HRegion region,SegmentedRollForward.Action rollForwardAction){
        ...
        CheckpointResolver checkpointResolver=getCheckpointResolver(region);
        return new TxnRegion(region,
                rollForward,
                resolver,
                TransactionStorage.getTxnSupplier(),
    ...
    private static CheckpointResolver getCheckpointResolver(HRegion region){
        AsyncCheckpointResolver acr = checkpointResolver;
        if(acr==null)
            acr = initializeCheckpointResolver();

        Region partition=new Region(region);
        sharedCheckpointResolver.addResolver(partition); //make sure the region has been registered

        return acr.checkpointResolver(partition);
    }
    ...
    private static AsyncCheckpointResolver initializeCheckpointResolver(){
        synchronized(lock){
            AsyncCheckpointResolver acr = checkpointResolver;
            if(acr==null){
                DataStore dataStore=TxnDataStore.getDataStore();
                TxnSupplier supplier = TransactionStorage.getTxnSupplier();
                IgnoreTxnCacheSupplier ignoreSupplier = TransactionStorage.getIgnoreTxnSupplier();
                sharedCheckpointResolver = new SharedCheckpointResolver(dataStore,supplier,ignoreSupplier,
                        SIConstants.readResolverThreads);
                acr = checkpointResolver = new AsyncCheckpointResolver(SIConstants.readResolverThreads,
                        SIConstants.readResolverQueueSize,
                        sharedCheckpointResolver);
            }
            return acr;
        }
    }
    private static class DiscardingTransactionalRegion implements TransactionalRegion{
        ...
        public DDLFilter ddlFilter(Txn ddlTxn) throws IOException{
            return delegate.ddlFilter(ddlTxn);
        }
        ...
        public boolean rowInRange(byte[] row){
            return delegate.rowInRange(row);
        }
        ...
        public void updateWriteRequests(long writeRequests){
            delegate.updateWriteRequests(writeRequests);
        }

        @Override
        public void updateReadRequests(long readRequests){
            delegate.updateReadRequests(readRequests);
        }
        ...
        public TxnSupplier getTxnSupplier(){
            return delegate.getTxnSupplier();
        }

        @Override
        public ReadResolver getReadResolver(){
            return delegate.getReadResolver();
        }
        ...
        public void close(){
            ...
            if(count==0){
                ...
                //remove from checkpoint resolver
                sharedCheckpointResolver.removeResolver(delegate.unwrapPartition());
            }
        }
