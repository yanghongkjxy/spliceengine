...
public abstract class SqlBuilder {
    ...
    public void createUniqueConstraints(Table table) throws IOException {
        for (int i = 0; i < table.getUniqueConstraintCount(); ++i) {
            UniqueConstraint uniqueConstraint = table.getUniqueConstraint(i);
            createUniqueConstraint(table, uniqueConstraint);
        }
    }

    public void createUniqueConstraint(Table table, UniqueConstraint uniqueConstraint) throws IOException {
        print("ALTER TABLE ");
        printlnIdentifier(getTableName(table));
        printIndent();
        print("ADD CONSTRAINT ");
        print(uniqueConstraint.getName());
        print(" UNIQUE (");
        int n = uniqueConstraint.getColumnCount();
        for (int i = 0; i < n; ++i) {
            print(uniqueConstraint.getColumn(i).getName());
            if (i != n - 1)
                print(", ");
        }
        print(")");
