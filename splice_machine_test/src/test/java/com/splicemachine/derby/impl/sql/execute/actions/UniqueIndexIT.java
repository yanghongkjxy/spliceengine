...
public class UniqueIndexIT extends SpliceUnitTest{
    ...
    public void testCanUseUniqueIndex() throws Exception{
        try(Statement s=conn.createStatement()){
            s.executeUpdate("create unique index "+INDEX_A+" on "+TABLE_A+"(name)");
//        new SpliceIndexWatcher(TABLE_A,CLASS_NAME, INDEX_A,CLASS_NAME,"(name)",true).starting(null);
            ...
            ResultSet resultSet=s.executeQuery(format("select * from %s where name = '%s'",TABLE_A,name));
    ...
    public void testCanCreateIndexFromExistingData() throws Exception{
        ...
        try(Statement s=conn.createStatement()){
            ...
            //create the index
            s.executeUpdate("create unique index "+INDEX_B+" on "+TABLE_B+"(name)");
//            new SpliceIndexWatcher(TABLE_B,CLASS_NAME,INDEX_B,CLASS_NAME,"(name)",true).starting(null);
    ...
    public void testCanCreateIndexFromExistingDataAndThenAddData() throws Exception{
        try(Statement s=conn.createStatement()){
            ...
            //create the index
            s.executeUpdate("create unique index "+INDEX_C+" on "+TABLE_C+"(name)");
//            new SpliceIndexWatcher(TABLE_C,CLASS_NAME,INDEX_C,CLASS_NAME,"(name)",true).starting(null);
            ...
            //now check that we can get data out for the proper key
            ResultSet resultSet=s.executeQuery(format("select * from %s where name = '%s'",TABLE_C,name));
            List<String> results=Lists.newArrayListWithExpectedSize(1);
            while(resultSet.next()){
                String retName=resultSet.getString(1);
                int val=resultSet.getInt(2);
    ...
    public void testViolateUniqueConstraint() throws Exception{
        try(Statement s=conn.createStatement()){
            s.executeUpdate("create unique index "+INDEX_D+" on "+TABLE_D+"(name)");
            String name="sfines";
            int value=2;
    ...
    public void testCanDropIndex() throws Exception{
        try(Statement s=conn.createStatement()){
            ...
            s.executeUpdate("create unique index "+INDEX_E+" on "+TABLE_E+"(name)");
            String name="sfines";
            int value=2;
            String insertStatement=format("insert into %s (name,val) values ('%s',%s)",TABLE_E,name,value);

            s.executeUpdate(insertStatement);
            //verify that we get a unique constraint violation
            try{
                s.executeUpdate(insertStatement);
            ...
            s.executeUpdate("drop index "+INDEX_E);

            //validate that we can add duplicates now
            s.executeUpdate(insertStatement);

            ResultSet resultSet=s.executeQuery(format("select * from %s where name = '%s'",TABLE_E,name));
            List<String> results=Lists.newArrayListWithExpectedSize(1);
            while(resultSet.next()){
                String retName=resultSet.getString(1);
                int val=resultSet.getInt(2);
    ...
    public void testCanDeleteIndexWithNulls() throws Exception{
        try(Statement s=conn.createStatement()){
            s.executeUpdate("create index "+INDEX_F+" on "+TABLE_I+"(val)");
//            new SpliceIndexWatcher(TABLE_I,CLASS_NAME,INDEX_F,CLASS_NAME,"(val)",false).starting(null);
        }
        try(PreparedStatement ps=methodWatcher.prepareStatement(String.format("insert into %s (name,val) values (?,?)",TABLE_I))){
    ...
    public void testCanDeleteUniqueIndexWithoutNulls() throws Exception{
        try(Statement s=conn.createStatement()){
            s.executeUpdate("create unique index "+INDEX_F+" on "+TABLE_J+"(val)");
    ...
    public void testCanDeleteUniqueIndexWithoutNulls2() throws Exception{
        ...
        String tableName="T2";
        try(Statement s = conn.createStatement()){
            s.executeUpdate("create table "+tableName+"(name varchar(40),val int)");
            s.executeUpdate("create unique index "+indexName+" on "+tableName+"(val)");
    ...
    public void testCanCreateDescendingIndex() throws Exception{
        ...
        String tableName="T";
        try(Statement s = conn.createStatement()){
            s.executeUpdate("create table "+ tableName+" (c1 int, c2 smallint)");
            ...
            s.executeUpdate("create unique index "+indexName+" on "+ tableName+"(c2 desc, c1)");
    ...
    public void testCanDeleteThenInsertEntryInTransaction() throws Exception{
        try(Statement s = conn.createStatement()){
            s.executeUpdate("create unique index "+ INDEX_F+" on "+TABLE_F+"(name)");
            String name="sfines";
            int value=2;
            s.execute(format("insert into %s (name, val) values ('%s', %s)",TABLE_F,name,value));
            s.execute(format("delete from %s",TABLE_F,name));
            s.execute(format("insert into %s (name, val) values ('%s', %s)",TABLE_F,name,value));
            ResultSet rs=s.executeQuery(format("select * from %s where name = '%s'",TABLE_F,name));
            List<String> results=Lists.newArrayListWithExpectedSize(1);
            while(rs.next()){
    ...
    public void testCanInsertThenDeleteEntryInTransaction() throws Exception{
        try(Statement s = conn.createStatement()){
            ...
            insertThenDelete(s,TABLE_M);
        }
    }

    private void insertThenDelete(Statement s,String tableName) throws Exception{
        String name="sfines";
        int value=2;
        ...
        ResultSet rs=s.executeQuery(format("select * from %s where name = '%s'",tableName,name));
    ...
    public void testCanInsertThenDeleteThenInsertAgainInTransaction() throws Exception{
        try(Statement s = conn.createStatement()){
            s.executeUpdate("create unique index "+ INDEX_K+" on "+TABLE_K+"(name)");
            insertThenDelete(s,TABLE_K);
            insertThenDelete(s,TABLE_K);
        }
    }
    ...
    public void testCanUpdateEntryIndexChanges() throws Exception{
        try(Statement s = conn.createStatement()){
            s.executeUpdate("create unique index "+INDEX_G+" on "+TABLE_G+" (name)");
            String name="sfines";
            int value=2;
            ...
            ResultSet rs=s.executeQuery(format("select * from %s where name = '%s'",TABLE_G,newName));
            List<String> results=Lists.newArrayListWithExpectedSize(1);
            while(rs.next()){
                String retName=rs.getString(1);
                int val=rs.getInt(2);
    ...
    public void testUpdateUniqueIndex() throws Exception{
        ...
        try(Statement s = conn.createStatement()){
            s.executeUpdate("create table "+TABLE_2+"(id int, name varchar(10))");
            s.executeUpdate("create unique index "+ INDEX_2 +" on "+ TABLE_2+"(id)");
            ...
            ResultSet rs2=s.executeQuery("select * from t2 --SPLICE-PROPERTIES index="+INDEX_2+" \n where name='able'");
            ...
            try{
                ...
                // expected
                assertTrue("Expected SQLIntegrityConstraintViolationException",
                        SQLIntegrityConstraintViolationException.class.isAssignableFrom(e.getClass()));
...
//                        SQLIntegrityConstraintViolationException.class.getSimpleName(),e.getClass().getSimpleName());
            ...
            ResultSet rs=s.executeQuery("select * from t2 where name='baker'");
            List<String> results=Lists.newArrayListWithExpectedSize(1);
            while(rs.next()){
    ...
    public void testUpdateCompoundIndex() throws Exception{
        ...
        try(Statement s = conn.createStatement()){
            s.executeUpdate("create table "+TABLE_3+"(a varchar(5) primary key, b varchar(5), c varchar(5),d varchar(5))");
            s.executeUpdate("create unique index "+INDEX_3+" on "+TABLE_3+"(a,b desc, c)");
            s.execute("insert into "+TABLE_3+" values('A','A','A','A'), ('B','B','B','B'), ('C','C','C','C')");

            // update a previously null unique column to non-null value
            s.execute("update "+TABLE_3+" set a='M',b='M',c='M' where d='C'");
            // make sure we can see it via the index
            ResultSet rs2=s.executeQuery("select * from "+TABLE_3+" --SPLICE-PROPERTIES index="+INDEX_3+" \n where a='M'");
